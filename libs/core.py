...
from libs.exceptions import YtccException, BadURLException, ChannelDoesNotExistException, \
...
def _get_unix_ts(ts):
    d = datetime.datetime.strptime( ts, "%Y-%m-%dT%H:%M:%S+00:00" )
    ut = d.timestamp()
    return ut
    ...
    * ``set_include_watched_filter``
    ...
    def update_archive(self) -> None:
        ...
        for channel in channels:
            ...
            self.database.update_channel_last_check(channel.yt_channelid, ts,"true")
    ...
    def add_channel(self, displayname: str, dldir: str, channel_url: str) -> bool:

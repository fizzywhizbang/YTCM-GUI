...
from libs import core, arguments, terminal, _
from libs.database import Video
from libs.exceptions import BadConfigException, ChannelDoesNotExistException, \
...
try:
    ...
    COLUMN_FILTER = [core.config.table_format.getboolean("ID"),
...
DOWNLOAD_PATH = ""
HEADER_ENABLED = True
TABLE_HEADER = [_("ID"), _("Date"), _("Channel"), _("Title"), _("URL"), _("Downloaded")]
...
class Action(Enum):
    ...
    MARK_WATCHED = (_("Mark as downloaded"), terminal.Keys.F4, COLORS.prompt_mark_watched)
...
def download_video(video: Video, audio_only: bool = False) -> None:
    ...
    success = core.download_video(video=video, path=DOWNLOAD_PATH, audio_only=audio_only, publisherID=publisherID, videotitle=video.title)
    if not success:
...
def update_one(channel: str):
    core.update_one(channel)

@register_option("updatearchive")
...
def export_channels(file: BinaryIO) -> None:
    core.export_channels(file)


@register_option("version", exit=True)
def version() -> None:
    import libs
    print("ytcm version " + libs.__version__)
    print()
    print("Copyright (C) 2015-2019  " + libs.__author__)
...
def run() -> None:
    ...
    option_names = [
        ...
        "list", "download", "jdownload", "mark_downloaded"
    ]
